...
localStorage.setItem("Father", 1);
localStorage.setItem("Mother", 1);
localStorage.setItem("Daughter", 1);
localStorage.setItem("Full brother",1);
localStorage.setItem("Paternal grandfather",1);
...
alert("Number of family members in family array: " + family.length);
...
//submit : anHeir -> list of eligibile/ineligible heirs, their share percentage, and the dollar amount
//go through heirs and figure out who is eligble heir and who is not. 
//put eligible heirs in eligibleHeir array and ineligible in ineligibleHeir array.
//eligible heirs get reduced share or are universal heirs: determine.
...
window.alert(eligibleHeirs.length + " and " + ineligibleHeirs.length);

for(i = 0; i < eligibleHeirs.length; i++){
  window.alert("Number of eligible heirs: " + eligibleHeirs[i]);
}
for(j = 0; j < ineligibleHeirs.length; j++){
  window.alert("Number of ineligible heirs: " + ineligibleHeirs[j]);
}



//loop through storage, if heir is not blocked, add to eligibleHeirs, (remove from localStorage too?)
//if blocked, add to ineligibleHeirs




/*while(property > 0 && eligibleHeirs.length > 0){
  //find the shares of respective the eligible heirs and return it as percentage and dollar amount
  //Show who among the heirs was blocked from inheritance.
}*/
